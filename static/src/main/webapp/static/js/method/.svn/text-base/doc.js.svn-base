 ...
 */
(function(){
	window.Top = (typeof Top == "undefined")?{}:Top;
	...
	Event.onDOMReady(function(){
		//alert("22");
		var pres = YAHOO.util.Dom.get("bd").getElementsByTagName("pre");
		 pres && Dom.batch(pres,function(el){			
			codeFormat(el);
		});	
		setTimeout(function(){
	...
	function codeFormat(objId) {
		var pre = Dom.get(objId),
			js_source = pre.innerHTML.replace(/^\s+/, ''),
			tabsize = 4,
			tabchar = ' ';
		js_source = js_source.replace(/&lt;/g,"<");
		js_source = js_source.replace(/&gt;/g,">");
		
		if (js_source && Dom.hasClass(pre,"xml;")) {
		  js_source = style_html(js_source, tabsize, tabchar, 80);
		  js_source = js_source.replace(/\</g,"&lt;");
		  js_source = js_source.replace(/\>/g,"&gt;");
		  
		  
		  //js_source = TB.common.escapeHTML(js_source);
		  
		  if(pre.tagName.toLowerCase() == "pre" && YAHOO.env.ua.ie){
			pre.outerHTML = '<pre class="brush: xml;">'+js_source+'</pre>';
			//alert(2);
		  }else{
			pre.innerHTML = js_source;
		  }
		} else {
		  //alert(1);
		  js_source = js_beautify(js_source, tabsize, tabchar);
		  if(pre.tagName.toLowerCase() == "pre" && YAHOO.env.ua.ie){
			pre.outerHTML = '<pre class="brush: js;">'+js_source+'</pre>';
		  }else{
			pre.innerHTML = js_source;
		  }
		}
	  ...
	  function Parser() {
		...
		this.get_content = function () { //function to capture regular content between tags
		  ...
		  while (this.input.charAt(this.pos) !== '<') {
			...
			chars = this.input.charAt(this.pos);
			this.pos++;
			this.line_char_count++;
			
			
			if (this.Utils.in_array(chars, this.Utils.whitespace)) {
			  if (content.length) {
				space = true;
			  }
			  this.line_char_count--;
			...
			content.push(chars); //letter at-a-time (or string) inserted to an array
		  }
		...
		this.get_tag = function () { //function to get a full tag and parse its type
		  ...
		  do {
			...
			if (this.Utils.in_array(chars, this.Utils.whitespace)) { //don't want to insert unnecessary space
			  space = true;
			  this.line_char_count--;
			  continue;
			}
			...
			if (chars === "'" || chars === '"') {
			  if (!content[1] || content[1] !== '!') { //if we're in a comment strings don't get treated specially
				chars += this.get_unformatted(chars);
				space = true;
			  }
			}
			
			if (chars === '=') { //no space before =
			  space = false;
			}
			
			if (content.length && content[content.length-1] !== '=' && chars !== '>'
			...
			content.push(chars); //inserts character at-a-time (or string)
		  } while (chars !== '>');
