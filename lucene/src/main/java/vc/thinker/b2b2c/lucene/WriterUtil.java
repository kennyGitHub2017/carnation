 ...
 * 
 * <p>
 * Description:系统Lucene indexWriter实例管理器，第一次初始化时候进行一次实例化处理，此后一直单例使用，除非gc回收WriteUtil
...
public class WriterUtil {
	...
	private static final String GOODS = System.getProperty("thinkervcb2b2c.root") + File.separator + "luence"
			+ File.separator + "goods";
	private static final String GROUP = System.getProperty("thinkervcb2b2c.root") + File.separator + "luence"
			+ File.separator + "groupgoods";
	private static final String LIFE = System.getProperty("thinkervcb2b2c.root") + File.separator + "luence"
	...
	static {
		try {
			...
			IndexWriterConfig iwc1 = new IndexWriterConfig(Version.LUCENE_4_10_0, LuceneUtil.getAnalyzer());
			...
			IndexWriterConfig iwc2 = new IndexWriterConfig(Version.LUCENE_4_10_0, LuceneUtil.getAnalyzer());
			...
			IndexWriterConfig iwc3 = new IndexWriterConfig(Version.LUCENE_4_10_0, LuceneUtil.getAnalyzer());
			...
			indexWriter = new IndexWriter(FSDirectory.open(new File(GOODS)), iwc1);
			indexWriter_group = new IndexWriter(FSDirectory.open(new File(GROUP)), iwc2);
			indexWriter_life = new IndexWriter(FSDirectory.open(new File(LIFE)), iwc3);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
