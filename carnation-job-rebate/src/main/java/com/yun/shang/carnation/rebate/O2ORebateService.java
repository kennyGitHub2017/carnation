...
public class O2ORebateService extends RebateServiceBasic {
	...
	public synchronized boolean buyGoodsRebate(Long sellerId, Long orderId) {
		...
		// 订单BV（利润值）
		BigDecimal orderAddAmount = RebateTools.getO2OrderBvamout(orderId);
		String orderNo = groupOrderBO.getOrderId();
		Long orderUserId = groupOrderBO.getUserId() == null ? 0 : groupOrderBO.getUserId();
		// CN运营商下单分利润
		//RoleRebate cnRoleRebate = new RepeatRebate(orclService, userAccountService, moneyService);
		//cnRoleRebate.rebate(orderUserId, sellerId, orderId, orderNo, orderAddAmount, null);
		// 会员层面分润
