...
public class FormUserConsentHandler extends AbstractUserConsentHandler {
	...
	@Override
	public void handleUserConsent(HttpServletRequest request, HttpServletResponse response,
			FilterChain chain, String authStateValue, String returnUri, Client client) throws IOException,
			ServletException {
	...
	private void processInitial(HttpServletRequest request, ServletResponse response, FilterChain chain,
		...
		List<AccessToken> tokens = accessTokenRepository.findByClientId(client.getId());
		if (!CollectionUtils.isEmpty(tokens)) {
			// If another token is already present for this resource owner and client, no new consent should
			// be requested
			List<String> grantedScopes = tokens.get(0).getScopes(); // take the scopes of the first access
																	// token found.
