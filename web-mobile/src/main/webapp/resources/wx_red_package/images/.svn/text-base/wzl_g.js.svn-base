$(function(){
    ...
    $('#msg1').on(touchstart, function(){
        if(isTouch1){
         ...
         /*   stopAnotherVoice('msg3',_jbrVoice2);
            ...
            $('#msg1 .point').hide();
            $('#msg1 .voice>img').attr('src',loadingPath+'voice.gif');
            //添加播放完成事件
            _jbrVoice1 = createjs.Sound.play('1');
            _jbrVoice1.addEventListener("complete", handleClick2);
        }else{
 ...
 // 点击跳转
   $('#redpackage').on(touchstart, function(){
