...
//拼接日历
    ...
    $(".indate").attr("data-value",startStr).html(startStr.split("-")[1]+"月"+startStr.split("-")[2]+"日");
	$(".outdate").attr("data-value",endStr).html(endStr.split("-")[1]+"月"+endStr.split("-")[2]+"日");
	...
	dateList_cld.tap(function(){
		...
		if(active==0){
			...
			if(origin==now){
				...
				$(".indate").attr("data-value",dataValue1).html(dataValue1.split("-")[1]+"月"+dataValue1.split("-")[2]+"日");
				$(".outdate").attr("data-value",dataValue2).html(dataValue2.split("-")[1]+"月"+dataValue2.split("-")[2]+"日");
				
				var todayIndex=dateList_cld.index($(".cld .page-content .cld-item li[data-day="+todayStr+"]"));
				$("#des1").html(makeDateJump(todayIndex,origin,dataValue1));
				$("#des2").html(makeDateJump(todayIndex,now,dataValue2));				
				$("#dayCount").html(now-origin);
