...
function showDialog() {
  ...
  var type=arguments[3];//0为倒计时提示框，1为确认框（包含2个按钮，点击确定执行回调）,2为提示确认框（只含有一个确认按钮）,3为带输入框的
  ...
  jQuery("#sure").click(function(){
	  	var inputValue="";
	  	if(document.getElementById("input_enter")) inputValue=jQuery("#input_enter").val();
		jQuery("#"+id).remove();
