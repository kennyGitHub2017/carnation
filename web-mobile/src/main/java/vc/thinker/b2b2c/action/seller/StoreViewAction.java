...
//	private UserTools userTools;
	...
	public ModelAndView store(HttpServletRequest request, HttpServletResponse response, Long ugcId,
			String id, String orderBy, String orderType, MyPage<GoodsBO> page,
			String ex_price_flag,String stock,String freeShip) {
		
		logger.debug("<<<<<<<<<< orderBy:"+orderBy+" orderType:"+orderType+" ex_price_flag:"+ex_price_flag+" stock:"+stock+" freeShip:"+freeShip);
		...
		//包邮
		String goodsFreeShip = null;
		if(freeShip!=null && freeShip.trim().length() > 0)
		{
			goodsFreeShip = "1";
		}
...
//			store = this.storeService.getObjByProperty(null, "store_second_domain", secondDomain);
		...
		if (store == null) {
			...
			mv = new JModelAndView("default/storeDetailed.html", configService.getSysConfig(), 1, request,
					response);
			if (store.getStoreStatus() == 15) {
				...
				//add by yuleijia on 2017.4.20
				vo.setGoodsTransfee(goodsFreeShip);  //包邮
				vo.setHasStock(goodStock);  //有库存
				page.setPageSize(9);
				this.goodsService.selectGoodsListByVO(page, vo);
				CommUtil.saveIPageList2ModelAndView("", "", "", page, mv);

				logger.debug(">>>>>>> size:"+page.getResult().size());
				...
				/*mv.addObject("ex_price_flag", ex_price_flag);
				mv.addObject("goodStock", goodStock);
				mv.addObject("freeShip", goodsFreeShip);*/
...
//				}
	...
	@RequestMapping("/get_store_goods.htm")
	public void get_store_goods(HttpServletRequest request, HttpServletResponse response, Long ugcId,
			String id, String orderBy, String orderType, MyPage<GoodsBO> page,String ex_price_flag,String storeID,String stock,String freeShip)
	{
		String result = "";
		GoodsVO vo = new GoodsVO();
		...
		vo.setExPriceFlag(exPriceFlag);
		page.setPageSize(9);
		vo.setStoreId(Long.parseLong(storeID));
		this.goodsService.selectGoodsListByVO(page, vo);
		...
		Long uid = SecurityUserHolder.getCurrentUserId()!=null?SecurityUserHolder.getCurrentUserId():0L;
		/**
		 * 会员登陆后 检查是否为CN用户 CN用户 显示新增商品BV值 add by yuleijia 2017.2.20
		 */
		UserAccountBO account1 = new UserAccountBO();
		...
		{
			String goods_url = webPath+"/goods_"+good.getId()+".htm";
			String goodsMainPhotoPath = good.getGoodsMainPhotoPath();
			String goodName = good.getGoodsName();
			BigDecimal goodsShowPrice = good.getGoodsShowPrice();
			BigDecimal thisPrice = BigDecimal.ZERO;
			BigDecimal cnSelfAddPrice = BigDecimal.ZERO;
			if(good.getInventoryType().equals("spec") && good.getGoodsType()==1)
			{
				thisPrice = good.getGoodsPrice().add(good.getSelfAddPrice());
			}
			else
			{
				thisPrice = good.getGoodsPrice();
			}
			if(isCN && good.getExPriceFlag()==1 && good.getCnSelfAddPrice().compareTo(BigDecimal.ZERO)==1)
			{
				cnSelfAddPrice = good.getCnSelfAddPrice();
			}
			json = new JSONObject();
			json.element("goods_url", goods_url);
			json.element("goodsMainPhotoPath", goodsMainPhotoPath);
			json.element("goodName", goodName);
			json.element("goodsShowPrice", goodsShowPrice);
			json.element("thisPrice", thisPrice);
			json.element("cnSelfAddPrice", cnSelfAddPrice);
			json.element("isCN", isCN);
			jsonArr.add(json);
		}
