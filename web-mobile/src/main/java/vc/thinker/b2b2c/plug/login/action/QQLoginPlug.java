package vc.thinker.b2b2c.plug.login.action;
...
//	
	...
	public void qq_login_api(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String auth_url = "https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id="
				+ this.configService.getSysConfig().getQqLoginId() + "&redirect_uri=" + redirect_uri
				+ "&state=thinkervc&scope=get_user_info";
	...
	public String qq_login_bind(HttpServletRequest request, HttpServletResponse response, String code) {
		...
		String token_url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id="
				+ this.configService.getSysConfig().getQqLoginId() + "&client_secret="
				+ this.configService.getSysConfig().getQqLoginKey() + "&code=" + code + "&redirect_uri="
				+ redirect_uri;
		// access_token=1CA359B424836978AAA1424B83C1B5A3&expires_in=7776000&refresh_token=88E49894ECE227B7BEFE58877B292BA3
		String[] access_token_callback = CommUtil.null2String(this.getHttpContent(token_url, "UTF-8", "GET"))
				.split("&");
		...
		// );
		String me_callback = CommUtil.null2String(this.getHttpContent(me_url, "UTF-8", "GET"))
...
//		}
	...
	/**
	 * * 完成QQ账号绑定，默认账号密码为123456，到这里用户已经自动登录，用户可以选择修改密码，或者绑定已有商城用户，如绑定现有用户， 需要输入用户密码进行确认，密码正确后，
	 * 自动完成绑定并用现有用户账户登录
	 * 
	 * 
	 * @param request
	...
	public String qq_login_bind_finish(HttpServletRequest request, HttpServletResponse response,
			String userName, String password, String bind_already) {
...
//		}
	...
	public static void main(String[] args) {
		...
		config.setQqLoginId("100359491");
		config.setQqLoginKey("a34bcaef0487e650238983abc0fbae7c");
		String redirect_uri = CommUtil.encode("http://www.thinker.vc/qq_login_bind.htm");
		String auth_url = "https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id="
				+ config.getQqLoginId() + "&redirect_uri=" + redirect_uri
				+ "&state=thinkervc&scope=get_user_info";
		System.out.println(auth_url);

		String token_url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id="
				+ config.getQqLoginId() + "&client_secret=" + config.getQqLoginKey()
