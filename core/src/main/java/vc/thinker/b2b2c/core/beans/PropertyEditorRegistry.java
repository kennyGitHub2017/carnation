...
import vc.thinker.b2b2c.core.beans.propertyeditors.ByteArrayPropertyEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.CharArrayPropertyEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.CharacterEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.ClassArrayEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.ClassEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.CustomBooleanEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.CustomCollectionEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.CustomNumberEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.DateEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.LocaleEditor;
import vc.thinker.b2b2c.core.beans.propertyeditors.PropertiesEditor;
...
public class PropertyEditorRegistry {
	...
	/**
	 * Determine the property type for the given property path. Called by <code>findCustomEditor</code> if no
	 * required type has been specified, to be able to find a type-specific editor even if just given a
	 * property path.
	 * <p>
	 * Default implementation always returns <code>null</code>. BeanWrapperImpl overrides this with the
	 * standard <code>getPropertyType</code> method as defined by the BeanWrapper interface.
	 * 
	 * @param propertyPath
	...
	/**
	 * Get custom editor for the given type. If no direct match found, try custom editor for superclass (which
	 * will in any case be able to render a value as String via <code>getAsText</code>).
	 * 
	...
	/**
	 * Guess the property type of the specified property from the registered custom editors (provided that
	 * they were registered for a specific type).
	 * 
	 * @param propertyName
	 ...
	 *            the nested property path of the target registry, if any. If this is non-null, only editors
	 *            registered for a path below this nested property will be copied.
	 */
	...
	/**
	 * Add property paths with all variations of stripped keys and/or indexes. Invokes itself recursively with
	 * nested paths
	 * 
	 * @param strippedPaths
	...
	/**
	 * Holder for a registered custom editor with property name. Keeps the PropertyEditor itself plus the type
