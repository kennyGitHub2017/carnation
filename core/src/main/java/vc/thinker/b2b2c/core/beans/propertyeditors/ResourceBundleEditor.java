...
import vc.thinker.b2b2c.core.beans.Assert;
...
public class ResourceBundleEditor extends PropertyEditorSupport {
	 ...
	 * {@link #setAsText(String) converting from a String}.
	 */
	...
	public void setAsText(String text) throws IllegalArgumentException {
		...
		if (indexOfBaseNameSeparator == -1) {
			...
			if (!StringUtils.hasText(baseName)) {
				throw new IllegalArgumentException("Bad ResourceBundle name : received '" + text
						+ "' as argument to 'setAsText(String value)'.");
			}
			String localeString = rawBaseName.substring(indexOfBaseNameSeparator + 1);
			Locale locale = StringUtils.parseLocaleString(localeString);
			bundle = (StringUtils.hasText(localeString)) ? ResourceBundle.getBundle(baseName, locale)
