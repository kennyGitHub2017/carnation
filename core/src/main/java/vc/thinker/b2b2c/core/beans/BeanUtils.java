...
import vc.thinker.b2b2c.core.beans.exception.BeansException;
...
public abstract class BeanUtils {
	...
	/**
	 * Find a method with the given method name and the given parameter types, declared on the given class or
	...
	/**
	 * Find a method with the given method name and minimal parameters (best case: none), declared on the
	 * given class or one of its superclasses. Prefers public methods, but will return a protected, package
	 * access, or private method too.
	 * <p>
	 * Checks <code>Class.getMethods</code> first, falling back to
	 * <code>findDeclaredMethodWithMinimalParameters</code>. This allows to find public methods without issues
	 ...
	 *             if methods of the given name were found but could not be resolved to a unique method with
	 *             minimal parameters
	...
	/**
	 * Find a method with the given method name and minimal parameters (best case: none), declared on the
	 * given class or one of its superclasses. Will return a public, protected, package access, or private
	 * method.
	 * <p>
	...
	/**
	 * Find a method with the given method name and minimal parameters (best case: none) in the given list of
	 * methods.
	 * 
	 * @param methods
	...
	/**
	 * Parse a method signature in the form <code>methodName[([arg_list])]</code>, where <code>arg_list</code>
	 * is an optional, comma-separated list of fully-qualified type names, and attempts to resolve that
	 * signature against the supplied <code>Class</code>.
	 * <p>
	 * When not supplying an argument list (<code>methodName</code>) the method whose name matches and has the
	 * least number of parameters will be returned. When supplying an argument type list, only the method
	 * whose name and argument types match will be returned.
	 * <p>
	 * Note then that <code>methodName</code> and <code>methodName()</code> are <strong>not</strong> resolved
	 * in the same way. The signature <code>methodName</code> means the method called <code>methodName</code>
	 * with the least number of arguments, whereas <code>methodName()</code> means the method called
	 * <code>methodName</code> with exactly 0 arguments.
	 * <p>
	...
	/**
	 * Find a JavaBeans <code>PropertyDescriptor</code> for the given method, with the method either being the
	 * read method or the write method for that bean property.
	 * 
	 * @param method
	...
	public static String canonicalPropertyName(String propertyName) {
		...
		while (searchIndex != -1) {
			...
			if (keyStart != -1) {
				...
				if (keyEnd != -1) {
					String key = buf.substring(keyStart + PropertyAccessor.PROPERTY_KEY_PREFIX.length(),
							keyEnd);
	...
	/**
	 * Check if the given class represents a "simple" property, i.e. a primitive, a String, a Class, or a
	 * corresponding array. Used to determine properties to check for a "simple" dependency-check.
	 * 
	 ...
	 * <p>
	 * Note: The source and target classes do not have to match or even be derived from each other, as long as
	 * the properties match. Any bean properties that the source bean exposes but the target bean does not
	...
	/**
	 * Copy the property values of the given source bean into the given target bean, only setting properties
	 * defined in the given "editable" class (or interface).
	 * <p>
	 * Note: The source and target classes do not have to match or even be derived from each other, as long as
	 * the properties match. Any bean properties that the source bean exposes but the target bean does not
	 * will silently be ignored.
