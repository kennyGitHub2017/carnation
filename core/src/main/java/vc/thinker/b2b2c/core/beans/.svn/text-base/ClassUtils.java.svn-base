...
package vc.thinker.b2b2c.core.beans;
...
/**
 * Miscellaneous class utility methods. Mainly for internal use within the framework; consider Jakarta's
 * Commons Lang for a more comprehensive suite of utilities.
 * 
 * @author Keith Donald
 * @author Rob Harrop
...
public class ClassUtils extends ClassLoader {
	...
	/**
	 * Return a default ClassLoader to use (never <code>null</code>). Returns the thread context ClassLoader,
	 * if available. The ClassLoader that loaded the ClassUtils class will be used as fallback.
	 * <p>
	 * Call this method if you intend to use the thread context ClassLoader in a scenario where you absolutely
	 * need a non-null ClassLoader reference: for example, for class path resource loading (but not
	 * necessarily for <code>Class.forName</code>, which accepts a <code>null</code> ClassLoader reference as
	 * well).
	 * 
	...
	/**
	 * Return whether the {@link Class} identified by the supplied name is present and can be loaded. Will
	 * return <code>false</code> if either the class or one of its dependencies is not present or cannot be
	 * loaded.
	 */
	 ...
	 * ClassLoader that loaded the ClassUtils class as fallback.
	 * 
	 * @param name
	...
	/**
	 * Return the qualified name of the given class: usually simply the class name, but component type class
	 * name + "[]" for arrays.
	 * 
	 * @param clazz
	 *            the class
	...
	/**
	 * Return the number of methods with a given name (with any argument types), for the given class and/or
	 * its superclasses. Includes non-public methods.
	 * 
	 * @param clazz
	 *            the clazz to check
	 * @param methodName
	...
	/**
	 * Does the given class and/or its superclasses at least have one or more methods (with any argument
	 * types)? Includes non-public methods.
	 * 
	 * @param clazz
	 *            the clazz to check
	 * @param methodName
	...
	/**
	 * Check if the given class represents a primitive wrapper, i.e. Boolean, Byte, Character, Short, Integer,
	...
	/**
	 * Check if the given class represents a primitive (i.e. boolean, byte, char, short, int, long, float, or
	 * double) or a primitive wrapper (i.e. Boolean, Byte, Character, Short, Integer, Long, Float, or Double).
	 */
	...
	/**
	 * Check if the given class represents an array of primitives, i.e. boolean, byte, char, short, int, long,
	 * float, or double.
	 */
	...
	/**
	 * Check if the given class represents an array of primitive wrappers, i.e. Boolean, Byte, Character,
	...
	/**
	 * Determine if the given target type is assignable from the given value type, assuming setting by
	 * reflection. Considers primitive wrapper classes as assignable to the corresponding primitive types.
	 * 
	 * @param targetType
	 *            the target type
	 * @param valueType
	...
	public static boolean isAssignable(Class targetType, Class valueType) {
		...
		return (targetType.isAssignableFrom(valueType) || targetType.equals(primitiveWrapperTypeMap
				.get(valueType)));
	}
	 ...
	 * <code>Class.getResource</code> by prepending a slash ('/') to the return value. Built by taking the
	 * package of the specified class file, converting all dots ('.') to slashes ('/'), adding a trailing
	 * slash if necesssary, and concatenating the specified resource name to this. <br/>
	 * As such, this function may be used to build a path suitable for loading a resource file that is in the
	 * same package as a class file, although {@link org.springframework.core.io.ClassPathResource} is usually
	 ...
	 * i.e., all dots ('.') are replaced by slashes ('/'). Neither a leading nor trailing slash is added. The
	 * result could be concatenated with a slash and the name of a resource, and fed directly to
	 * ClassLoader.getResource(). For it to be fed to Class.getResource, a leading slash would also have to be
	 * prepended to the return value.
	 * 
	 * @param clazz
	 *            the input class. A null value or the default (empty) package will result in an empty string
	 *            ("") being returned.
	...
	/**
	 * Return all interfaces that the given object implements as array, including ones implemented by
	 * superclasses.
	 * 
	 * @param object
	 ...
	 */
	public static Class loadClass(String name, String path) throws ClassNotFoundException {
		try {
			...
			return classUtils.defineClass(name, classData, 0, classData.length);
