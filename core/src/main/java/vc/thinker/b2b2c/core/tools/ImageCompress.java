...
public class ImageCompress extends Frame {
	...
	public static void main(String[] args) {
		...
		if (gui.equals("gui")) {
			...
			ImageCompress.ImageScale(fileName, fileName + "-s." + getFileExt(fileName).toLowerCase(), 160,
					160);
	...
	/**
	 * createUI() creates the user controls and lays out the window. It also creates the event handlers (as
	 * inner classes) for the user controls.
	 */
	...
	public static void ImageScale(String sourceImg, String targetImg, int width, int height) {
		try {
			...
			// Make a BufferedImage from the Image.
			BufferedImage mBufferedImage = new BufferedImage(imageWidth, imageHeight,
					BufferedImage.TYPE_INT_RGB);
			Graphics2D g2 = mBufferedImage.createGraphics();
			g2.drawImage(image, 0, 0, imageWidth, imageHeight, Color.white, null);
			g2.dispose();
