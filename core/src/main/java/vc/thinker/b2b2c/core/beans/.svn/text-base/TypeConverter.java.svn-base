...
class TypeConverter {
	 ...
	 * @param requiredType
	 *            the type we must convert to (or <code>null</code> if not known, for example in case of a
	 *            collection element)
	 * @param methodParam
	...
	public Object convertIfNecessary(Object oldValue, Object newValue, PropertyDescriptor descriptor)
		...
		return convertIfNecessary(descriptor.getName(), oldValue, newValue, descriptor.getPropertyType(),
				descriptor, new MethodParameter(descriptor.getWriteMethod(), 0));
	}
	 ...
	 *             if type conversion failed
	 */
	protected Object convertIfNecessary(String propertyName, Object oldValue, Object newValue,
			Class requiredType, PropertyDescriptor descriptor, MethodParameter methodParam)
			throws IllegalArgumentException {
	...
	protected Collection convertToTypedCollection(Collection original, String propertyName,
		...
		Collection convertedCopy = CollectionFactory.createApproximateCollection(original.getClass(),
				original.size());
