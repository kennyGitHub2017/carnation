...
final class CachedIntrospectionResults {
	...
	/**
	 * Map keyed by class containing CachedIntrospectionResults. Needs to be a WeakHashMap with WeakReferences
	 * as values to allow for proper garbage collection in case of multiple class loaders.
	 */
	 ...
	 * <P>
	 * We don't want to use synchronization here. Object references are atomic, so we can live with doing the
	 * occasional unnecessary lookup at startup only.
	 * 
	 * @param beanClass
	...
	public static CachedIntrospectionResults forClass(Class beanClass) throws BeansException {
		...
		if (results == null) {
			...
			logger.debug("Class [" + beanClass.getName() + "] is " + (!cacheSafe ? "not " : "")
					+ "cache-safe");
	 ...
	 * <p>
	 * Many thanks to Guillaume Poirier for pointing out the garbage collection issues and for suggesting this
	 * solution.
	 * 
	 * @param clazz
