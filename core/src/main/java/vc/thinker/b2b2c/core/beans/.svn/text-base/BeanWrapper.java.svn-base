...
import vc.thinker.b2b2c.core.beans.exception.MethodInvocationException;
import vc.thinker.b2b2c.core.beans.exception.NotReadablePropertyException;
import vc.thinker.b2b2c.core.beans.exception.NotWritablePropertyException;
import vc.thinker.b2b2c.core.beans.exception.NullValueInNestedPathException;
import vc.thinker.b2b2c.core.beans.exception.PropertyException;
import vc.thinker.b2b2c.core.beans.exception.TypeMismatchException;
...
public class BeanWrapper extends AbstractPropertyAccessor {
	...
	/**
	 * Create new empty BeanWrapperImpl. Wrapped instance needs to be set afterwards. Registers default
	 * editors.
	 * 
	 ...
	 *            whether to register default editors (can be suppressed if the BeanWrapper won't need any
	 *            type conversion)
	 * @see #setWrappedInstance
	 */
	...
	/**
	 * Switch the target object, replacing the cached introspection results only if the class of the new
	 * object is different to that of the replaced object.
	 * 
	 * @param object
	 *            new target
	 */
	...
	public PropertyDescriptor getPropertyDescriptor(String propertyName) throws BeansException {
		...
		if (pd != null) {
			...
			throw new PropertyException(getRootClass(), this.nestedPath + propertyName, "No property '"
					+ propertyName + "' found");
		}
	}

	/**
	 * Internal version of getPropertyDescriptor: Returns <code>null</code> if not found rather than throwing
	 * an exception.
	 */
	...
	private String getFinalPath(BeanWrapper bw, String nestedPath) {
		...
		return nestedPath
				.substring(PropertyAccessorUtils.getLastNestedPropertySeparatorIndex(nestedPath) + 1);
	}
	 ...
	 * <p>
	 * Note: Caching nested BeanWrappers is necessary now, to keep registered custom editors for nested
	 * properties.
	 * 
	 ...
	 * <p>
	 * Default implementation creates a BeanWrapperImpl instance. Can be overridden in subclasses to create a
	 * BeanWrapperImpl subclass.
	 * 
	 * @param object
	 *            object wrapped by this BeanWrapper
	 * @param nestedPath
	...
	private PropertyTokenHolder getPropertyNameTokens(String propertyName) {
		...
		if (!keys.isEmpty()) {
			tokens.canonicalName += PROPERTY_KEY_PREFIX
					+ StringUtils
							.collectionToDelimitedString(keys, PROPERTY_KEY_SUFFIX + PROPERTY_KEY_PREFIX)
					+ PROPERTY_KEY_SUFFIX;
			...
			if (tokens.keys != null) {
				...
				for (int i = 0; i < tokens.keys.length; i++) {
					...
					if (value == null) {
						...
						Object convertedMapKey = this.typeConverterDelegate.convertIfNecessary(null, null,
								key, mapKeyType);
	...
	private void setPropertyValue(PropertyTokenHolder tokens, Object newValue) throws BeansException {
		...
		if (tokens.keys != null) {
			...
			if (propValue == null) {
				...
				try {
					Object convertedValue = this.typeConverterDelegate.convertIfNecessary(propertyName,
							oldValue, newValue, requiredType);
					Array.set(propValue, Integer.parseInt(key), convertedValue);
				} catch (IllegalArgumentException ex) {
					PropertyChangeEvent pce = new PropertyChangeEvent(this.rootObject, this.nestedPath
				...
				try {
					// IMPORTANT: Do not pass full property name in here - property editors
					// must not kick in for map keys but rather only for map values.
					convertedMapKey = this.typeConverterDelegate.convertIfNecessary(null, null, key,
							mapKeyType);
				} catch (IllegalArgumentException ex) {
					PropertyChangeEvent pce = new PropertyChangeEvent(this.rootObject, this.nestedPath
							+ propertyName, oldValue, newValue);
			...
			try {
				...
				Object convertedValue = isProx ? newValue : this.typeConverterDelegate.convertIfNecessary(
