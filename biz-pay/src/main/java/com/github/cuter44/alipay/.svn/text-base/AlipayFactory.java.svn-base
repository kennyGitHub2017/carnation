...
public class AlipayFactory {
	 ...
	 */
	public AlipayFactory(String resource) throws MissingResourceException {
		this();

		try {
			this.conf.load(new InputStreamReader(AlipayFactory.class.getResourceAsStream(resource), "utf-8"));

			return;
		} catch (Exception ex) {
			MissingResourceException mrex = new MissingResourceException("Failed to load conf resource.",
					AlipayFactory.class.getName(), resource);
			mrex.initCause(ex);
	...
	/**
	 * Default constructor which looking for a .properties resource in order below: 1.
	 * System.getProperty("com.github.cuter44.alipay.alipay_properties") 2.
	 * AlipayFactory.class.getResource("/alipay.properties") Properties MUST stored in utf-8.
	 */
	// public AlipayFactory()
	...
	/**
	 * return default instance which load config from <code>/alipay.properties</code>. If you are binding
	 * multi-instance of AlipayFactory in your application, DO NOT use this method.
	 */
	...
	/**
	 * @deprecated Please use <code>getDefaultInstance()</code> instead. This method now forwarded to
	 *             <code>getDefaultInstance()</code>
	 */
	...
	public TradeCreateByBuyer newTradeCreateByBuyer() {
		return (new TradeCreateByBuyer(new Properties(this.conf)));
	}

	public TradeCreateByBuyer newTradeCreateByBuyer(Properties p) {
		return (new TradeCreateByBuyer(buildConf(p, this.conf)));
	}

	public SendGoodsConfirmByPlatform newSendGoodsConfirmByPlatform() {
		return (new SendGoodsConfirmByPlatform(new Properties(this.conf)));
	}

	public SendGoodsConfirmByPlatform newSendGoodsConfirmByPlatform(Properties p) {
		return (new SendGoodsConfirmByPlatform(buildConf(p, this.conf)));
	}

	public WapTradeCreateDirect newWapTradeCreateDirect() {
		return (new WapTradeCreateDirect(new Properties(this.conf)));
	}

	public WapAuthAndExecute newWapAuthAndExecute() {
		return (new WapAuthAndExecute(new Properties(this.conf)));
	}

	public WapAuthAndExecute newWapAuthAndExecute(Properties p) {
		return (new WapAuthAndExecute(buildConf(p, this.conf)));
	}

	public CreateDirectPayByUser newCreateDirectPayByUser() {
		return (new CreateDirectPayByUser(new Properties(this.conf)));
	}

	public CreateDirectPayByUser newCreateDirectPayByUser(Properties p) {
		return (new CreateDirectPayByUser(buildConf(p, this.conf)));
	}

	public WapTradeCreateDirect newWapTradeCreateDirect(Properties p) {
		return (new WapTradeCreateDirect(buildConf(p, this.conf)));
	}

	public CreateDirectPayByUserBank newCreateDirectPayByUserBank() {
		return (new CreateDirectPayByUserBank(new Properties(this.conf)));
	}

	public CreateDirectPayByUserBank newCreateDirectPayByUserBank(Properties p) {
		return (new CreateDirectPayByUserBank(buildConf(p, this.conf)));
	}

	public NotifyVerify newNotifyVerify() {
		return (new NotifyVerify(new Properties(this.conf)));
	}

	public NotifyVerify newNotifyVerify(Properties p) {
		return (new NotifyVerify(buildConf(p, this.conf)));
	}
	...
	public BatchTransNotify newBatchTransNotify() {
		return (new BatchTransNotify(new Properties(this.conf)));
	}
	...
	public BatchTransNotify newBatchTransNotify(Properties p) {
		return (new BatchTransNotify(buildConf(p, this.conf)));
	}

	public RefundFastpayByPlatformPwd newRefundFastpayByPlatformPwd() {
		return (new RefundFastpayByPlatformPwd(new Properties(this.conf)));
	}

	public RefundFastpayByPlatformPwd newRefundFastpayByPlatformPwd(Properties p) {
		return (new RefundFastpayByPlatformPwd(buildConf(p, this.conf)));
	}

	public CreatePartnerTradeByBuyer newCreatePartnerTradeByBuyer() {
		return (new CreatePartnerTradeByBuyer(new Properties(this.conf)));
	}

	public CreatePartnerTradeByBuyer newCreatePartnerTradeByBuyer(Properties p) {
		return (new CreatePartnerTradeByBuyer(buildConf(p, this.conf)));
	}
