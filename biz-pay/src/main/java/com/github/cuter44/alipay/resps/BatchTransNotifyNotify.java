...
import com.github.cuter44.alipay.helper.AlipayAccount;
...
public class BatchTransNotifyNotify extends NotifyBase {
	...
	public static final List<String> KEYS_PARAM_NAME = Arrays
			.asList("batch_no", "fail_details", "notify_id", "notify_time", "notify_type", "pay_account_no",
					"pay_user_id", "pay_user_name", "success_details");
	...
	public String getBatchNo() {
		return (this.respProp.getProperty(KEY_BATCH_NO));
	}

	public AlipayAccount getPayUser() {
		AlipayAccount aa = new AlipayAccount(this.respProp.getProperty(KEY_PAY_USER_ID),
				this.respProp.getProperty(KEY_PAY_USER_NAME), this.respProp.getProperty(KEY_PAY_ACCOUNT_NO));
	...
	public List<PaymentItem> getSuccessDetails() {
		...
		for (String item : items) {
			try {
				...
				PaymentItem pi = new PaymentItem(values[0], values[1], values[2], Double.valueOf(values[3]),
						"S".equals(values[4]), null, values[6], df.parse(values[7]));
