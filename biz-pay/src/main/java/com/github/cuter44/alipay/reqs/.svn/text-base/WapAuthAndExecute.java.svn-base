...
public class WapAuthAndExecute extends WapRequestBase {
	// CONSTANTS
	public static final List<String> KEYS_PARAM_NAME = Arrays.asList("format", "partner", "req_data",
			"sec_id", "service", "v");

	public static final List<String> TAGNAMES_REQ_DATA = Arrays.asList("request_token");
	...
	public WapAuthAndExecute(Properties aConf) {
		super(aConf);
		this.setProperty("service", "alipay.wap.auth.authAndExecute").setProperty("format", "xml")
				.setProperty("v", "2.0");
	...
	// SIGN
	@Override
	public WapAuthAndExecute sign() throws UnsupportedEncodingException, IllegalStateException {
