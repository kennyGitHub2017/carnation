...
public class NotifyVerify extends WebRequestBase {
	// CONSTANTS
	public static final List<String> KEYS_PARAM_NAME = Arrays.asList("service", "notify_id", "partner");
	...
	public String toURL() {
		try {
			return (this.toUnsignedURL(KEYS_PARAM_NAME, "utf-8"));
	...
	@Override
	public NotifyVerifyResponse execute() throws AlipayException {
		try {
			String params = Request.Get(this.toURL()).execute().returnContent().asString();

			return (new NotifyVerifyResponse(params, null));
		} catch (Exception ex) {
