...
public class SendGoodsConfirmByPlatform extends WebRequestBase {
	...
	public static final List<String> KEYS_PARAM_NAME = Arrays.asList("_input_charset",
			"create_transport_type", "invoice_no", "logistics_name", "partner", "seller_ip", "service",
			"trade_no", "transport_type");
	...
	// SIGN
	@Override
	public SendGoodsConfirmByPlatform sign() throws UnsupportedEncodingException {
	...
	public SendGoodsConfirmByPlatformResponse execute() {
		try {
			...
			String content = Request.Get(this.toURL()).execute().returnContent().asString();

			prop.putAll(parseXML(content));

			if (prop.getProperty(KEY_ERROR) != null)
				throw (new AlipayException(prop.getProperty(KEY_ERROR).toUpperCase().replace(' ', '_')));
