...
/**
 * <b>Note that this implementation is not synchronized.</b> If multiple threads access an
 * <code>RoyaltyList</code> instance concurrently, and at least one of the threads modifies the list
...
public class RoyaltyList extends ArrayList<IRoyaltyItem> {
	 ...
	 *                on size%gt;10, according to alipay spec.
	 */
	@Override
	...
	// VALIDATE
	/**
	 * validate if this RoyaltyList can be executed. <br />
	 * Note that this check is not always accurate, since alipay did not state which rounding strategy
	 * applied. Another problem is, due to the precision lost of floating-point, potential crash may happened.
	 * To avoid this, please don't attempt to touch the critical value, or skip this check if you are
	 * confident.
	 * 
	 * @param income
	 *            amount that the buyer actually payed.
	 * @param fee
	 *            fee charged by alipay, in ratio. Ask your customer service for accurate value.
	 * @return this, if passed.
	 * @exception IllegalStateException
	 *                if there is a obvious leak.
	 */
	public RoyaltyList validate(double income, double fee) throws IllegalStateException {
		...
		for (IRoyaltyItem i : this) {
			...
			map.put(out, map.get(out) - amount);
			map.put(in, map.get(in) + amount);
		}
	 ...
	 */
	public RoyaltyList validate(double income) throws IllegalStateException {
		return (this.validate(income, 0.0012));
	}
	...
	public String toString() {
		...
		for (IRoyaltyItem i : this) {
			...
			if (i.getPayAccount() != null)
				sb.append(i.getPayAccount()).append('^');

			sb.append(i.getAccount()).append('^').append(String.format("%.2f", i.getAmount())).append('^')
					.append(i.getMemo());
		}
