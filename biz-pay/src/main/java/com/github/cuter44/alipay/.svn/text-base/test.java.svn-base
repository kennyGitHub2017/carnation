...
import com.github.cuter44.alipay.constants.LogisticsPayment;
import com.github.cuter44.alipay.constants.LogisticsType;
import com.github.cuter44.alipay.helper.Logistics;
import com.github.cuter44.alipay.helper.PaymentDetailList;
import com.github.cuter44.alipay.helper.PaymentItem;
...
public class test {
	public static void browserOpen(String url) {
		try {
			...
			System.out
					.println("Your OS seems not supporting a desktop browser, which is required... (´・ω・｀)");
			ex.printStackTrace();
		}
	}
	...
	public static String demoTradeCreateByBuyer() {
		...
		try {
			...
			RequestBase req = factory.newTradeCreateByBuyer().setOutTradeNo("test" + rand.nextLong())
			// equals to:
			// .setProperty("out_trade_no", "test"+rand.nextLong())
					...
					.setLogistics(
							new Logistics(LogisticsType.EXPRESS, 0.01,
									LogisticsPayment.BUYER_PAY_AFTER_RECEIVE),
							new Logistics(LogisticsType.EMS, 0.01, LogisticsPayment.BUYER_PAY),
							new Logistics(LogisticsType.POST, 0.01, LogisticsPayment.SELLER_PAY))
	...
	public static ResponseBase demoWapTradeCreateDirect() {
		...
		try {
			...
			RequestBase req = factory.newWapTradeCreateDirect()
					...
					.setProperty("out_trade_no", "test" + rand.nextLong()).setProperty("total_fee", "0.01")
	...
	public static String demoCreateDirectPayByUser() {
		...
		try {
			...
			RequestBase req = factory
					.newCreateDirectPayByUser()
					.setRoyalty(
							new RoyaltyList(new PaymentItem("468859947@qq.com", 1.00, "一级分润"),
					...
					.setProperty("out_trade_no", "test" + rand.nextLong()).setProperty("subject", "直接到帐支付测试")
					.setProperty("payment_type", "1").setProperty("total_fee", "2.00")
					.setProperty("notify_url", "http://weixin.uutime.cn/nyagalin/gateway");
	...
	public static String demoCreateDirectPayByUserBank() {
		...
		try {
			...
			RequestBase req = factory.newCreateDirectPayByUserBank()
					.setProperty("out_trade_no", "test" + rand.nextLong()).setProperty("subject", "银行网关支付测试")
					.setProperty("payment_type", "1").setProperty("total_fee", "0.01")
	...
	public static void demoTradeCreateByUserAndSendGoods() {
		...
		try {
			...
			RequestBase req1 = factory.newTradeCreateByBuyer()
					.setProperty("out_trade_no", "test" + rand.nextLong())
					.setProperty("subject", "标准双接口支付测试").setProperty("payment_type", "1")
					.setProperty("logistics_type", "EMS").setProperty("logistics_fee", "0.01")
					.setProperty("logistics_payment", "SELLER_PAY").setProperty("price", "0.01")
			...
			RequestBase req2 = factory.newSendGoodsConfirmByPlatform().setProperty("trade_no", trade_no)
					.setProperty("transport_type", "EXPRESS").setProperty("logistics_name", "黑猫宅配便");
	...
	public static String demoBatchTransNotify() {
		...
		try {
			...
			RequestBase req = factory
					.newBatchTransNotify()
					.setPaymentDetail(
							new PaymentDetailList(new PaymentItem(Long.toString(abs(rand.nextLong())),
									"cuter44@qq.com", "吴嘉林", 0.01, "支付宝商家付款测试, 么么哒"), new PaymentItem(Long
									.toString(abs(rand.nextLong())), "18825180163", "陈晓杰", 0.01,
									"支付宝商家付款测试, 么么哒"),
							// new PaymentItem(Long.toString(abs(rand.nextLong())), "468859947@qq.com", "黄文杰",
							// 0.01, "支付宝商家付款测试, 么么哒"),
							// new PaymentItem(Long.toString(abs(rand.nextLong())), "18814127437", "陈蔓青",
							// 0.01, "支付宝商家付款测试, 么么哒"),
							// new PaymentItem(Long.toString(abs(rand.nextLong())), "cuter44@qq.com", "吴嘉林",
							// -0.01, "金额不正确的用例"),
									new PaymentItem(Long.toString(abs(rand.nextLong())), "468859947@qq.com",
											"0xDEADDEAD", 0.01, "名字不正确的用例"), new PaymentItem(Long
											.toString(abs(rand.nextLong())), "0xDEADDEAD", "陈蔓青", 0.01,
											"帐号不正确的用例")))
	...
	public static String demoRefundFastpayByPlatformPwd() {
		...
		try {
			...
			RequestBase req = factory.newCreateDirectPayByUser()
					.setProperty("out_trade_no", "test" + rand.nextLong()).setProperty("subject", "即时到帐退款测试")
					.setProperty("payment_type", "1").setProperty("total_fee", "0.01")
					.setProperty("notify_url", "http://weixin.uutime.cn/nyagalin/gateway");
			...
			RequestBase req2 = factory.newRefundFastpayByPlatformPwd()
					...
					.setProperty("batch_num", "1").setProperty("detail_data", trade_no + "^0.01^退款测试");
	...
	public static String demoCreatePartnerTradeByBuyer() {
		...
		try {
			...
			RequestBase req = factory.newCreatePartnerTradeByBuyer()
					.setProperty("out_trade_no", "test" + rand.nextLong())
					.setProperty("seller_email", "13622833735").setProperty("subject", "平台商担保支付测试")
					.setProperty("payment_type", "1").setProperty("logistics_type", "EXPRESS")
					.setProperty("logistics_fee", "0").setProperty("logistics_payment", "BUYER_PAY")
					.setProperty("price", "0.01").setProperty("quantity", "1")
	...
	public static String donateToAuthor() {
		...
		try {
			...
			RequestBase req = factory
					.newCreateDirectPayByUser()
					.setRoyalty(
							new RoyaltyList(new PaymentItem("cuter44@qq.com", 1.00, "辛苦了这是小鱼干(alipay-sdk)")))
					.setProperty("out_trade_no", "nyaguru" + rand.nextLong())
					.setProperty("subject", "捐赠给支付宝SDK作者").setProperty("payment_type", "1")
	...
	public static void main(String[] args) {
		...
		// TESTCASE 8
		browserOpen(demoBatchTransNotify());
