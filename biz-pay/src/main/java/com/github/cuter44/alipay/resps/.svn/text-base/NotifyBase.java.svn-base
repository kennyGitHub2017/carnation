...
public class NotifyBase extends ResponseBase {
	 ...
	 *                if <code>charset</code> is incorrect.
	 */
	protected String signMD5(List<String> paramNames, String key, String charset)
		...
		StringBuilder sb = new StringBuilder().append(this.toQueryString(paramNames)).append(key);

		String sign = CryptoUtil.byteToHex(CryptoUtil.MD5Digest(sb.toString().getBytes(charset)));
	...
	public boolean verifyNotifySign(List<String> paramNames, Properties conf)
		...
		String calculated = this.sign(paramNames, this.getProperty("sign_type"), conf.getProperty("KEY"),
	 ...
	 */
	protected boolean verifyNotifySign(Properties conf) throws UnsupportedEncodingException,
	...
	/**
	 * @deprecated for decoupling, please use <code>verify(Properties prop)</code> instead Now this method is
	 *             forwarded to this.verify(factory.getConf())
	 */
	@Deprecated
	public boolean verify(AlipayFactory factory) throws UnsupportedEncodingException {
		return (this.verify(factory.getConf()));
	}
	...
	public String getNotifyId() {
		return (this.respProp.getProperty(KEY_NOTIFY_ID));
	}

	public NotifyType getNotifyType() {
		return (NotifyType.forName(this.respProp.getProperty(KEY_NOTIFY_TYPE)));
	}

	/**
	 * exception ParseException while format is incorrect
	 */
	public Date getNotifyTime() throws ParseException {
		return (new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(this.respProp.getProperty(KEY_NOTIFY_TIME)));
	}
