...
public abstract class WapRequestBase extends RequestBase {
	...
	// SIGN
	/**
	 * Default sign adapter It will grab <code>sign_type</code> and <code>sign</code>
	 */
	public RequestBase sign(List<String> paramNames) throws UnsupportedEncodingException {
	...
	protected String toSignedURL(List<String> paramNames, String charset) throws UnsupportedEncodingException {
		URLBuilder ub = new URLBuilder();
	...
	public ResponseBase execute() throws AlipayException {
		try {
			String params = Request.Get(this.toURL()).execute().returnContent().asString();

			Properties prop = parseHttpParam(params);
			if (prop.getProperty(KEY_RES_ERROR) != null)
				throw (new AlipayException(prop.getProperty(KEY_RES_ERROR_MSG).toUpperCase()
						.replace(' ', '_')));
			// else
