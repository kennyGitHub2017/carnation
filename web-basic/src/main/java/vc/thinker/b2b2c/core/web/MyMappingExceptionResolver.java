package vc.thinker.b2b2c.core.web;
...
public class MyMappingExceptionResolver extends AbstractHandlerExceptionResolver {
	...
	/**
	 * Set the mappings between exception class names and error view names. The exception class name can be a
	 * substring, with no wildcard support at present. A value of "ServletException" would match
	 * {@code javax.servlet.ServletException} and subclasses, for example.
	 * <p>
	 * <b>NB:</b> Consider carefully how specific the pattern is, and whether to include package information
	 * (which isn't mandatory). For example, "Exception" will match nearly anything, and will probably hide
	 * other rules. "java.lang.Exception" would be correct if "Exception" was meant to define a rule for all
	 * checked exceptions. With more unusual exception names such as "BaseBusinessException" there's no need
	 * to use a FQN.
	 * 
	 * @param mappings
	 *            exception patterns (can also be fully qualified class names) as keys, and error view names
	 *            as values
	 */
	...
	/**
	 * Set one or more exceptions to be excluded from the exception mappings. Excluded exceptions are checked
	 * first and if one of them equals the actual exception, the exception will remain unresolved.
	 * 
	...
	/**
	 * Set the HTTP status code that this exception resolver will apply for a given resolved error view. Keys
	 * are view names; values are status codes.
	 * <p>
	 * Note that this error code will only get applied in case of a top-level request. It will not be set for
	 * an include request, since the HTTP status cannot be modified from within an include.
	 * <p>
	...
	/**
	 * Returns the HTTP status codes provided via {@link #setStatusCodes(Properties)}. Keys are view names;
	 ...
	 * and if there is no status code mapping defined.
	 * <p>
	 * Note that this error code will only get applied in case of a top-level request. It will not be set for
	 * an include request, since the HTTP status cannot be modified from within an include.
	 * <p>
	 * If not specified, no status code will be applied, either leaving this to the controller or view, or
	 * keeping the servlet engine's default of 200 (OK).
	 * 
	 ...
	 * <p>
	 * This can be either set to a different attribute name or to {@code null} for not exposing an exception
	 * attribute at all.
	 * 
	 ...
	 * <p>
	 * May be overridden in subclasses, in order to apply specific exception checks. Note that this template
	 * method will be invoked <i>after</i> checking whether this resolved applies ("mappedHandlers" etc), so
	 ...
	 * @param handler
	 *            the executed handler, or {@code null} if none chosen at the time of the exception (for
	 ...
	 * {@link #setExceptionMappings "exceptionMappings"}, and finally using the {@link #setDefaultErrorView
	...
	protected String findMatchingViewName(Properties exceptionMappings, Exception ex) {
		...
		for (Enumeration<?> names = exceptionMappings.propertyNames(); names.hasMoreElements();) {
			...
			if (depth >= 0
					&& (depth < deepest || (depth == deepest && dominantMapping != null && exceptionMapping
	 ...
	 * <p>
	 * 0 means ex matches exactly. Returns -1 if there's no match. Otherwise, returns depth. Lowest depth
	 * wins.
	 */
	 ...
	 * @return the HTTP status code to use, or {@code null} for the servlet container's default (200 in case
	 *         of a standard error view)
	...
	/**
	 * Apply the specified HTTP status code to the given response, if possible (that is, if not executing
	 * within an include request).
	 * 
	 * @param request
	 *            current HTTP request
	 * @param response
