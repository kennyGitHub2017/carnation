...
<body>
	...
	<div class="mod-block">
		<div class="cont-width clearfix">
			...
			<div class="content-main col-xs-12">${ refereeArticle.content }</div>
			<div class="content-nav col-xs-12">
				<c:if test="${listArticlePrevious!=null}">
					<p>
						<a href="${yesPath}/news/${listArticlePrevious.id}">上一篇： ${ listArticlePrevious.title }</a>
					</p>
				</c:if>
				<c:if test="${listArticleNext!=null}">
					<p class="">
						<a href="${yesPath}/news/${listArticleNext.id}">下一篇： ${ listArticleNext.title }</a>
					</p>
				</c:if>
			</div>
		</div>
