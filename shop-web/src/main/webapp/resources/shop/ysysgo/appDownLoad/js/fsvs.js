...
;( function($){
	...
	$.fn.fsvs = function( options ) {
		...
		var mouseWheelHandler = function( ev ) {
			...
			var e = window.event || ev, wheely;
			
			wheely = ( e.wheelDelta || -e.detail || e.originalEvent.detail );
		...
		var slideCallback = function( index ) {
            var $this = $( options.selector, body ).eq( index );
			currentSlideIndex = index;
			options.afterSlide( index, $this);
		...
		var jQuerySlide = function( index ) {
            var $this = $( options.selector, body ).eq( index );
			options.beforeSlide( index,  $this);
			if( body.is( ':animated' ) ) {
				currentSlideIndex = index;
				body.stop();
			}
		...
		var app = {
			...
			addPagination : function() {
				...
				$( options.selector, body ).each( function(i) {
					...
					$('<li class="' + linkClass + '"><span><a href="javascript:;"></a></span></li>').appendTo( pagination );
				});
