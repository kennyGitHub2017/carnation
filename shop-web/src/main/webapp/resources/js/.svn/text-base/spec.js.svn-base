...
var spec_return=(function(){
	...
	$(".js-add-sku-group").click(function(){
		...
		if(!document.getElementById("select2-drop")){
			$.ajax({
			     type: 'POST',
			     anysc:false,
			     url: $("#webpath").val()+"/seller/get_spec_list.htm",
			    data: {},
	...
	$(document).delegate(".ui-popover .js-save","click",function(){
		...
		$.ajax({
		     type: 'POST',
		     anysc:false,
		     url: $("#webpath").val()+"/seller/goods_property_save_batch.htm",
	...
	$(".sku-group").delegate(".js-remove-sku-atom","click",function(){
		...
		$.ajax({
		     type: 'POST',
		     url: $("#webpath").val()+"/seller/goods_property_delete.htm",
		    data: {id:id},
		    success: function(data){
		    },
		    dataType: 'json'
	...
	.on(
			...
			jQuery.ajaxFileUpload({
							url:$("#webpath").val()+'/seller/goods_property_img_upload.htm?mark='+mark,
							fileElementId : [ eid ],
							fileFilter : ".jpg,.gif,.png",
							fileSize : 1048576,
							dataType : "text",
	...
	function inventory_type_change(){
		  ...
		  if(rv=="all"){
			...
			var goods_spec_ids="";
			  ...
			  if(goods_spec_ids!=""){
			   jQuery.post($("#webpath").val()+"/seller/goods_inventory.htm",{"goods_spec_ids":goods_spec_ids},function(data){
