...
public class WeixinPayViewAction {
	...
	private static final Logger log = LoggerFactory.getLogger(WeixinPayViewAction.class);
	...
	public ModelAndView wap_alipay_return(HttpServletRequest request, HttpServletResponse response)
		...
		// if (type.equals("goods")) {
		// if (main_order.getOrderStatus() <
		// OrderFormConstants.ORDER_STATUS_TO_BE_SHIPPED) {// 异步没有出来订单，则同步处理订单
		// List<OrderForm>
		// orderForms=this.orderFormService.completePay(main_order.getId(),
		...
		// }
		// if (type.equals("integral")) {
		// if (ig_order.getIgoStatus() <
		// IntegralGoodsOrderConstants.IGO_STATUS_HAVE_PAYED) {
	...
	public void wap_alipay_notify(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// OrderFormConstants.ORDER_STATUS_TO_BE_SHIPPED) {// 异步没有出来订单，则同步处理订单
		// List<OrderForm>
		// result=this.orderFormService.completePay(main_order.getId(),
		// trade_no, main_order.getUserId(),"支付宝在线支付");
	...
	public String weixin_qr_pay_notify(HttpServletRequest request, HttpServletResponse response)
		...
		// PayUtils.convertRequest(request.getInputStream());
		/*
		...
		if (resHandler.isValidSign(partner_key) == true) {
			if (resHandler.isWXsign(app_key) == true) {
				...
				if ("0".equals(trade_state)) {
					...
					//
					// if (main_order != null &&
					// main_order.getOrderStatus() <
					// OrderFormConstants.ORDER_STATUS_TO_BE_SHIPPED) {//
					// 异步没有出来订单，则同步处理订单
					// List<OrderForm>
					// result=this.orderFormService.completePay(main_order.getId(),
					...
					// if (ig_order != null && ig_order.getIgoStatus() <
					// IntegralGoodsOrderConstants.IGO_STATUS_HAVE_PAYED) {
					// this.integralGoodsOrderService.completePay(ig_order,
