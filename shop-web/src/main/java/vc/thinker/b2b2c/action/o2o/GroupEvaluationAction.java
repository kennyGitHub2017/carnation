...
public class GroupEvaluationAction {
	...
	public ModelAndView groupOrderEvaluation(long orderId, String type, HttpServletRequest request,
			HttpServletResponse response) {
		int flag = "down".equals(type) ? 2 : 1;
		...
		if (null == order || order.getOrderStatus() != 30) {
			...
			mv.addObject("url", CommUtil.getURL(request) + "/buyer/group.htm?orderType=" + flag);
			return mv;
		}
		...
		if (order.getStore() != null) {
			order.setStoreName(order.getStore().getStoreName());
			AlbumBO albumBO = albumService.getStoreAlbum(order.getStore().getCreateUserId());
			if (albumBO != null) {
				...
				if (accessory != null && StringUtils.isNotBlank(accessory.getPath())) {
					order.setGroupAccPath(accessory.getPath());
				}
			}
		}
		return mv;
