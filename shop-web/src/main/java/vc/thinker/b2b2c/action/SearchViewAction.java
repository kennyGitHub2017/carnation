...
//	private IGroupService groupService;
	...
	public ModelAndView search(HttpServletRequest request, HttpServletResponse response, String gc_id,
			String keyword, GoodsVO vo, MyPage<GoodsBO> page

	) throws UnsupportedEncodingException {
		ModelAndView mv = new JModelAndView("lucene/search_goods_list.html", configService.getSysConfig(), 1,
				request, response);
		...
		if (StringUtils.isNotEmpty(keyword)) {
			...
			if (gc_id != null && !gc_id.equals("")) {
				...
				query_gc = gc.getLevel() == 1 ? gc_id + "_*" : CommUtil.null2String(gc.getParentId()) + "_"
						+ gc_id;
				mv.addObject("gc_id", gc_id);
			}
			LuceneResult pList = null;
			if (sort != null) {
				/*pList = luceneUtil.search(keyword, CommUtil.null2Int(page.getCurrentPage()-1),
						CommUtil.null2String(vo.getGoodsInventory()), CommUtil.null2String(vo.getGoodsType()), query_gc, CommUtil.null2String(vo.getGoodsTransfee()),
						CommUtil.null2String(vo.getGoodsCod()), sort, null, null, null);*/
				pList = luceneUtil.search(GoodsDocument.class, keyword,
						...
						CommUtil.null2String(vo.getGoodsTransfee()), CommUtil.null2String(vo.getGoodsCod()),
						null, null, null, sort, null, 0, true, null, null);
			} else {
				pList = luceneUtil.search(GoodsDocument.class, keyword,
						CommUtil.null2Int(page.getCurrentPage() - 1),
						CommUtil.null2String(vo.getGoodsInventory()),
...
//		mv.addObject("goods_compare_list", goods_compare_list);
	...
	public void search_goodsclass(HttpServletResponse response, String keyword) {
		if (keyword == null || keyword.equals("")) {
			return;
		}
		List<GoodsClassBO> objs = this.goodsClassService.findRelatedGoodsClass(2, keyword);
		...
		if (objs.size() > 0) {
			...
			for (GoodsClassBO obj : objs) {
				...
				List<GoodsClassBO> childs = dicContent.getDicsConvertByParentCode(GoodsClassBO.class,
				...
				if (StringUtils.isNotBlank(str)) {
					...
					if (temp.length > 0) {
						...
						if (!hasSelectList.contains(sid)) {
							...
							GoodsClassBO bo = (GoodsClassBO) dicContent.getTreeDic(GoodsClassBO.class, sid);
							gcs.add(bo);
						}
					}
				}
		...
		if (CollectionUtils.isNotEmpty(stores)) {
			Collections.sort(stores, new Comparator<StoreBO>() {
				public int compare(StoreBO o1, StoreBO o2) {
					...
					if (l1 == l2) {
						if (store1.getPoint().getStoreEvaluate()
