...
public class IntegralOrderBuyerAction {
	...
	@SecurityMapping(title = "买家积分兑换", value = "/buyer/integral/swap.htm*", rtype = "buyer", rname = "用户中心", rcode = "user_center", rgroup = "用户中心")
	@RequestMapping("/buyer/integral/swap.htm")
	public ModelAndView integralSwap(String pwd, Integer exchangeIntegral, HttpServletResponse response) throws IOException{
		Integer RETURN_STATUS = null;
		...
		if(RETURN_STATUS == null){
			...
			IntegralSwapBO bo = userCustomerService.integralSwap(uid, pwd, exchangeIntegral);
			RETURN_STATUS = bo.getRETURN_STATUS();
		}
		response.getWriter().write(String.valueOf(RETURN_STATUS));
		return null;
	}
}
