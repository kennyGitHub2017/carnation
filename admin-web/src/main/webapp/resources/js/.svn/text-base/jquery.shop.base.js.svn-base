var pagestyle=function(){    
   jQuery("#workspace .left").css("width","12%")
   jQuery("#workspace .content").css({"width":"88%","height":jQuery(window).height()});
   try{ 
...
function openURL(args){
 ...
 if(type=="show"){
   ...
   if(jQuery(first_li).find("li").size() > 0)
   {
	   jQuery(first_li).find("ul").show();
	   var imgSrc = jQuery(first_li).find("img").attr("src");
	   jQuery(first_li).find("img").attr("src",imgSrc.replace("spread.jpg", "contract.jpg"));
	   first_li = jQuery(first_li).find("li:first");
	   first_a=jQuery(first_li).find("a:first");
   }
