...
function selectAll(obj){
	...
	var dataGroup=jQuery(obj).attr("data-group");
  	if(status){
  		if(dataGroup){
  			jQuery("#ListForm").find(":checkbox[id!="+id+"][data-group="+dataGroup+"]").prop("checked",true);
  		}else{
  			jQuery("#ListForm").find(":checkbox[id!="+id+"]").prop("checked",true);
  		}		
	}else{
		if(dataGroup){
			jQuery("#ListForm").find(":checkbox[id!="+id+"][data-group="+dataGroup+"]").prop("checked",false);
		}else{
			jQuery("#ListForm").find(":checkbox[id!="+id+"]").prop("checked",false);
		}	    	
	}
}
/* 
...
jQuery(document).ready(function(){
  ...
  tipStyle();
  //单独弹出一个编辑窗口
  jQuery("a[dialog_uri],input[dialog_uri],dt[dialog_uri]").click(function(e){
  
	var dialog_title=jQuery(this).attr("dialog_title");
    var dialog_id=jQuery(this).attr("dialog_id"); 
    var dialog_uri=jQuery(this).attr("dialog_uri") + "&dialogId=" + dialog_id;
	var dialog_height=jQuery(this).attr("dialog_height") || 0;
  ...
  jQuery("a[ck_dialog_uri],input[ck_dialog_uri],dt[ck_dialog_uri]").click(function(e){
      var mulitId="";
	  jQuery("#ListForm").find(":checkbox:checked").each(function(){
	    if(jQuery(this).val()!=""){	  
	      mulitId+=jQuery(this).val()+",";
...
if (jQuery.validator) {
	...
	jQuery.validator.addMethod("telNumber", function (value, element) {
	    return this.optional(element) || telNumber(value);
	}, "请正确输入您的电话号码");
}
