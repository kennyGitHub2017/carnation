		...
		this.showPicker = function(callback, engine, sideLength_) {
			...
			this.getAllColorArray = function() {
				...
				var baseArray = [ new RGB(255, 0, 0), new RGB(0, 255, 0),
						...
						new RGB(0, 255, 255), new RGB(255, 0, 255),
						new RGB(204, 255, 0), new RGB(153, 0, 255),
						new RGB(102, 255, 255), new RGB(51, 0, 0) ];
				mixColorArray = mixColorArray.concat(baseArray.slice(0,
						sideLength));
				...
				for (var i = 0; i < cornerColorArray.length; i++) {
					...
					for (var j = 0; j < sideLength; j++) {
						if (i < 3) {
							halfOfAllArray1[count] = gradientColor(
			...
			if (engine == "L") {
				...
				if (defaultSectionsWidth < minSectionsWidth) {
					$("#bigSections li,#bigLayout").width(
							minSectionsWidth / (sideLength * 3 + 2) - 2)
							.height(minSectionsHeight / (sideLength * 2) - 1);
					$("#bigpicker").height(minBigpickerHeight).width(
							minBigpickerWidth);
					$("#bigSections").height(minSectionsHeight).width(
							minSectionsWidth);
				} else {
					$("#bigSections").width(defaultSectionsWidth).height(
							sideLength * 2 * 11);
					$("#bigpicker").width(defaultSectionsWidth + 5).height(
							sideLength * 2 * 11 + 29);
				}

				$("#bigSections ul").find("li:last").css("border-bottom",
						"1px solid #000000");
				$("#bigSections ul:last li").css("border-right",
						"1px solid #000000");
				...
				// ��ʼ��ΪĬ����ʽ
				$("#bigSections").height(134).width(222).addClass(
						"bigpicker-bgimage").empty();
